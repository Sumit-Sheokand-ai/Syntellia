...
export default function LoginPage() {
  ...
  const signInWithPassword = async (event: FormEvent) => {
    ...
    if (signInError) { setError(signInError.message); setIsPending(false); return; }
  ...
  return (
    <div className="rounded-[24px] border border-white/[0.08] bg-[#0d0e1f]/80 p-8 shadow-[0_40px_100px_rgba(0,0,0,0.5)] backdrop-blur-xl">
      ...
      <div className="mb-7 flex items-end gap-6 border-b border-white/[0.08] pb-1">
        ...
        <Link href={`/auth/signup?next=${encodeURIComponent(nextPath)}`} className="pb-3 text-[14px] font-medium text-white/38 transition hover:text-white/65">
          Sign Up
        </Link>
      </div>

      <h1 className="text-[22px] font-bold text-white">Welcome back</h1>
      <p className="mt-1.5 text-[13px] text-white/45">Please enter your details to sign in.</p>
      ...
      {/* Form */}
      <form className="space-y-3.5" onSubmit={signInWithPassword} aria-busy={isPending}>
        <div>
          <label htmlFor="login-email" className="mb-1.5 block text-[12px] font-medium text-white/55">Email</label>
          <input
            id="login-email"
            type="email"
            required
        ...
        <div>
          <div className="mb-1.5 flex items-center justify-between">
            <label htmlFor="login-password" className="text-[12px] font-medium text-white/55">Password</label>
            <button type="button" className="text-[12px] font-medium text-[#7c3aed] transition hover:text-[#a78bfa]">
              Forgot password?
            </button>
          </div>
          <input
        ...
        <label className="flex cursor-pointer items-center gap-2.5">
          ...
          />
          <span className="text-[12px] text-white/45">Remember me for 30 days</span>
        </label>
        ...
        >
          {isPending ? t("auth.signingIn") : "Sign In"}
        </button>
      </form>

      <p className="mt-5 text-center text-[12px] text-white/40">
        Don&apos;t have an account?{" "}
        <Link href={`/auth/signup?next=${encodeURIComponent(nextPath)}`} className="font-semibold text-[#7c3aed] transition hover:text-[#a78bfa]">
          Sign up
        </Link>
      </p>
    </div>
